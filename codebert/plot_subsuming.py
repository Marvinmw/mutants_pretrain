...
def span_index(func, code_span, code_insert_span, tokenizer, args):
    ...
    if s_j > args.code_length-2:
        s_j = args.code_length-2
    if s_i > args.code_length-2:
        s_i = -1
        s_j = -1
...
def convert_examples_to_features(item):
    #source
    code1, code1_span, insert_code1_span,code2, code2_span, insert_code2_span, label,tokenizer, args, project, global_id=item
    #print(code1)
    cache={}
   
    for cid,func in enumerate([code1,code2]):
        #source
        source_tokens = tokenizer.tokenize(func, truncation=True, max_length=args.code_length-2)
        source_tokens =[tokenizer.cls_token]+source_tokens+[tokenizer.sep_token]
    ...
    code1_index = span_index(code1, code1_span, insert_code1_span, tokenizer, args)
    code2_index = span_index(code2, code2_span, insert_code2_span, tokenizer, args)
  ...
  #  print(cache)    
    ...
    return InputFeatures(source_tokens_1,source_ids_1,source_mask_1,code1_index,
                   source_tokens_2,source_ids_2,source_mask_2,code2_index, project,global_id, label )
...
def create_data( tokenizer, args, dataname, file_path='train', testproject=[]):
        ...
        with open(file_path, "r") as f:
            for line in f:
                ...
                data.append( (js['mutant_codes'], js['mutant_codes_span'], js['mutant_codes_span_insert_mask'],
                             js['original_codes'], js['original_codes_span'], js['original_codes_span_insert_mask'],
  ...
  #  data_stat={1:0, 0:0}
    ...
    json.dump({"train":tdata_stat,"train_ratio":tdata_stat[1]/(tdata_stat[0]+tdata_stat[1]),  "test":test_stat, 
        "test_ratio": test_stat[1]/(test_stat[0]+test_stat[1])}, open(stat_path, "w"), indent=4)
    ...
    valid_examples = train_examples[:int(len(train_examples)*0.2)]
...
def train(args, train_dataset,valid_examples, model, tokenizer):
    """ Train the model """
    ...
    for idx in range(args.epochs): 
        ...
        for step, batch in enumerate(bar):
            ...
            inputs_ids_2,attn_mask_2, input_span_2,
            labels)=[x.to(args.device)  for x in batch]
            model.train()
            loss,logits = model(inputs_ids_1,attn_mask_1, input_span_1,  inputs_ids_2,attn_mask_2, input_span_2 , labels)
...
def evaluate(args, model, tokenizer,examples, eval_when_training=False):
    #build dataloader
    ...
    for batch in eval_dataloader:    
        ...
        with torch.no_grad():
            lm_loss,logit = model(inputs_ids_1,attn_mask_1, input_span_1, inputs_ids_2,attn_mask_2, input_span_2,labels)
...
def main():
    ...
    if args.do_train:
        ...
        train(args, train_dataset,valid_examples, model, tokenizer)
